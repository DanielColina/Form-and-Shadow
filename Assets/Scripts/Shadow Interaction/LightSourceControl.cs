...
public class LightSourceControl : MonoBehaviour
{
	...
	void Awake ()
    {
        active = GetComponent<Light>().enabled;
	...
	public void TurnLightSource(bool turnClockwise)
    {
        ...
        m_LightSourceForward = transform.forward;
        CheckLightingDirection();
        ShadowCast[] shadowCastObjects = FindObjectsOfType<ShadowCast>();
        foreach (ShadowCast shadowCastObject in shadowCastObjects)
            shadowCastObject.CastShadowCollider(this);
    }
}
